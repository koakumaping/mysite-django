...
class CKEditor(forms.Textarea):
    ...
    class Media:
        js = (
            getattr(settings, 'CKEDITOR_PATH', 'js/ckeditor/ckeditor.js'),
        )
...
var CKeditor = new CKEDITOR.replace('%s',
{
customConfig : '',
%s
});
CKeditor.BasePath = "%sjs/ckeditor/";
</script>''' % (name, ck_attrs, settings.MEDIA_URL))
